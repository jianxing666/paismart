...
public class UploadController {
    ...
    public ResponseEntity<Map<String, Object>> uploadChunk(
        ...
        try {
            // 文件类型验证（仅在第一个分片时进行验证）
            if (chunkIndex == 0) {
                FileTypeValidationService.FileTypeValidationResult validationResult = fileTypeValidationService
                        .validateFileType(fileName);

                LogUtils.logBusiness("UPLOAD_CHUNK", userId, "文件类型验证结果: fileName=%s, valid=%s, fileType=%s, message=%s",
                        fileName, validationResult.isValid(), validationResult.getFileType(),
                        validationResult.getMessage());

                if (!validationResult.isValid()) {
                    LogUtils.logBusinessError("UPLOAD_CHUNK", userId, "文件类型验证失败: fileName=%s, fileType=%s",
                            new RuntimeException(validationResult.getMessage()), fileName,
            ...
            LogUtils.logBusiness("UPLOAD_CHUNK", userId,
                    "接收到分片上传请求: fileMd5=%s, chunkIndex=%d, fileName=%s, fileType=%s, contentType=%s, fileSize=%d, totalSize=%d, orgTag=%s, isPublic=%s",
            ...
            if (orgTag == null || orgTag.isEmpty()) {
                try {
                    ...
                    orgTag = primaryOrg;
                    LogUtils.logBusiness("UPLOAD_CHUNK", userId, "成功获取用户主组织标签: fileName=%s, orgTag=%s", fileName,
                            orgTag);
                } catch (Exception e) {
            ...
            LogUtils.logBusiness("UPLOAD_CHUNK", userId,
                    "分片上传成功: fileMd5=%s, fileName=%s, fileType=%s, chunkIndex=%d, 进度=%.2f%%",
            ...
            LogUtils.logBusinessError("UPLOAD_CHUNK", userId, "分片上传失败: fileMd5=%s, fileName=%s, chunkIndex=%d", e,
            ...
            String fileType = getFileType(fileName);
            LogUtils.logBusinessError("UPLOAD_CHUNK", userId,
                    "分片上传失败: fileMd5=%s, fileName=%s, fileType=%s, chunkIndex=%d", e, fileMd5, fileName, fileType,
                    chunkIndex);
            monitor.end("分片上传失败: " + e.getMessage());
            Map<String, Object> errorResponse = new HashMap<>();
            errorResponse.put("code", HttpStatus.INTERNAL_SERVER_ERROR.value());
    ...
    public ResponseEntity<Map<String, Object>> getUploadStatus(@RequestParam("file_md5") String fileMd5,
        ...
        try {
            ...
            try {
                ...
                LogUtils.logBusiness("GET_UPLOAD_STATUS", "system", "获取文件信息失败，使用默认值: fileMd5=%s, 错误=%s", fileMd5,
                        e.getMessage());
            }

            LogUtils.logBusiness("GET_UPLOAD_STATUS", "system", "获取文件上传状态: fileMd5=%s, fileName=%s, fileType=%s",
                    fileMd5, fileName, fileType);

            List<Integer> uploadedChunks = uploadService.getUploadedChunks(fileMd5, userId);
            int totalChunks = uploadService.getTotalChunks(fileMd5, userId);
            double progress = calculateProgress(uploadedChunks, totalChunks);

            LogUtils.logBusiness("GET_UPLOAD_STATUS", "system",
                    "文件上传状态: fileMd5=%s, fileName=%s, fileType=%s, 已上传=%d/%d, 进度=%.2f%%",
    ...
    public ResponseEntity<Map<String, Object>> mergeFile(
        ...
        try {
            ...
            // 检查文件完整性和权限
            LogUtils.logBusiness("MERGE_FILE", userId, "检查文件记录和权限: fileMd5=%s, fileName=%s", request.fileMd5(),
                    request.fileName());
            FileUpload fileUpload = fileUploadRepository
                    .findFirstByFileMd5AndUserIdOrderByCreatedAtDesc(request.fileMd5(), userId)
                    .orElseThrow(() -> {
            ...
            if (fileUpload.getStatus() == FileUpload.STATUS_COMPLETED) {
                LogUtils.logBusiness("MERGE_FILE", userId, "文件已完成合并，按幂等成功返回: fileMd5=%s, fileName=%s",
            ...
            LogUtils.logBusiness("MERGE_FILE", userId, "权限验证通过，开始合并文件: fileMd5=%s, fileName=%s, fileType=%s",
                    request.fileMd5(), request.fileName(), fileType);
            ...
            if (updatedRows == 0) {
                FileUpload latestFileUpload = fileUploadRepository
                        .findFirstByFileMd5AndUserIdOrderByCreatedAtDesc(request.fileMd5(), userId)
                        .orElseThrow(() -> new RuntimeException("文件记录不存在"));
                if (latestFileUpload.getStatus() == FileUpload.STATUS_COMPLETED) {
                    LogUtils.logBusiness("MERGE_FILE", userId, "文件已被其他请求合并完成，按幂等成功返回: fileMd5=%s, fileName=%s",
                            request.fileMd5(), request.fileName());
                    monitor.end("文件已完成合并");
                    return buildAlreadyMergedResponse(request.fileMd5());
                }
            ...
            // 合并文件
            LogUtils.logBusiness("MERGE_FILE", userId, "开始合并文件分片: fileMd5=%s, fileName=%s, fileType=%s, 分片数量=%d",
                    request.fileMd5(), request.fileName(), fileType, totalChunks);
            String objectUrl;
            try {
                objectUrl = uploadService.mergeChunks(request.fileMd5(), request.fileName(), userId);
            } catch (Exception mergeException) {
                fileUploadRepository.updateStatusIfCurrent(fileUpload.getId(), FileUpload.STATUS_MERGING,
                        FileUpload.STATUS_UPLOADING);
            ...
            LogUtils.logBusiness("MERGE_FILE", userId,
                    "创建文件处理任务: fileMd5=%s, fileName=%s, fileType=%s, orgTag=%s, isPublic=%s",
            ...
            LogUtils.logBusiness("MERGE_FILE", userId, "发送文件处理任务到RabbitMQ: exchange=%s, fileMd5=%s, fileName=%s",
                    rabbitMQConfig.getFileProcessingExchange(), request.fileMd5(), request.fileName());
            rabbitTemplate.convertAndSend(rabbitMQConfig.getFileProcessingExchange(),
                    rabbitMQConfig.getFileProcessingRoutingKey(), task);
            LogUtils.logBusiness("MERGE_FILE", userId, "文件处理任务已发送: fileMd5=%s, fileName=%s, fileType=%s",
                    request.fileMd5(), request.fileName(), fileType);
